<template>
  <v-container class="py-6" style="max-width: 1000px;">
    <div class="d-flex align-center justify-space-between mb-4">
      <div class="text-h5 font-weight-bold">Rank Requests</div>
    ...
    <v-alert v-if="!isLeader" type="warning" variant="tonal">
          ...
          <v-card-title class="d-flex align-center justify-space-between">
            <div>
              <div class="text-subtitle-1 font-weight-medium">
                {{ r.username }} → {{ prettyRank(r.requested_rank) }}
              </div>
...
import { authMe, listRankRequests, approveRankRequest, rejectRankRequest } from "../services/api";
...
function prettyRank(rank) {
  return String(rank || "member").replaceAll("_", " ").replace(/\b\w/g, (c) => c.toUpperCase());
}
...
async function load() {
  ...
  try {
    ...
    if (isLeader.value) {
      requests.value = await listRankRequests("pending");
    } else {
      requests.value = [];
    }
  } catch (e) {
    error.value = e?.message || "Failed to load rank requests.";
  } finally {
    loading.value = false;
  }
}
